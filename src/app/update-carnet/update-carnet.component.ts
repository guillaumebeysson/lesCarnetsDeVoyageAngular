...
export class UpdateCarnetComponent implements OnInit {
  ...
  pathFileImg = "http://localhost:8080/files/"; 
  ...
  constructor(private http: HttpClient, private carnetService: CarnetService,private route: ActivatedRoute, private router: Router, private userService: UserService) { }
  ...
  ngOnInit() {
    ...
    const idCarnetSelected: number = this.route.snapshot.params['id'];
    ...
    if (token) {
      this.userService.getUserById(this.userService.getIdFromToken(token)).subscribe((user) => {
        ...
        this.userService.getCarnetByUser(user?.id!).subscribe(res => {
          this.carnets = res;
          if(!this.carnets?.find(c => c.id == idCarnetSelected)){
            this.router.navigateByUrl('403')
          }
        })
        this.carnetService.getCarnetById(idCarnetSelected).subscribe(res => {
          this.carnet = res;
          this.imageUrl1 = this.pathFileImg + this.carnet.picture1;
          this.imageUrl2 = this.pathFileImg + this.carnet.picture2;
          this.imageUrl3 = this.pathFileImg + this.carnet.picture3;
        })
      })
    }
  ...
  validCarnet() {
    ...
    this.carnetService.updateCarnet(this.carnet, picture1File, picture2File, picture3File).subscribe({
      next: result => {
